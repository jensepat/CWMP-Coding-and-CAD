...
#os.environ['OPENMC_CROSS_SECTIONS'] = '/nfs/stak/users/spraguep/project/NucData/lib80x/cross_sections.xml'
...
graphite.add_s_alpha_beta('c_Graphite_10p')
...
water.add_s_alpha_beta('c_O_in_D2O')
...
flute_mix.add_s_alpha_beta('c_O_in_D2O')
...
molarity = 1.0392
NA = 0.602214  # Avogadro's in (atoms/mol) * (cm^2/b) for b-cm units

# 1.0392 M converted to atom density (atoms/b-cm)
# Formula: (molarity / 1000) * NA
np_atom_density = (molarity / 1000.0) * NA 

#  1.0392 M Aqueous Neptunium Complex Solution [NpO2(H2O)5]+
np_aqueous = openmc.Material(name="1.0392M Np Aqueous Solution")
np_aqueous.id = 93001
np_aqueous.depletable = True

np_aqueous.add_nuclide('Np237', np_atom_density)

# Water components (assuming ~55.5 mol/L of H2O)
# Plus the 5 waters in the [NpO2(H2O)5]+ complex
# At this molarity, H density is approx 0.066 atoms/b-cm
np_aqueous.add_element('H', 0.066, 'ao') 
np_aqueous.add_element('O', 0.035, 'ao')

np_aqueous.add_s_alpha_beta('c_H_in_H2O')

# Set density based on the sum of the specified atom densities
np_aqueous.set_density('sum')
