...
openmc.config['chain_file'] = '/nfs/stak/users/bichselc/Paul TRIGA/aqueous_test/chain_casl_pwr.xml' # This will have to be editted to your files for modifications
...
def ostr_model(n_particles, n_inactive, n_active, rod_heights, megawatt=False):
    ...
    # Volume Results
    h = 60.97          # Height of the liquid column (cm)
    R = 29.87          # Outer radius of the air/liquid space (cm)
    thickness = 0.47   # Target thickness (cm)

    # Equation: r = R - thickness
    r_calculated = R - thickness 

    # Calculate the resulting Volume in cm3 (mL)
    volume_cm3 = np.pi * h * (R**2 - r_calculated**2)
    mats.np_aqueous.volume = volume_cm3
    
    # Lazy Susan
    s_lazy_susan = RCC(center_base=(0, 0, 5.81), height=61.97, radius=34.369375, axis='z')
    s_ls_air_od = RCC(center_base=(0, 0, 6.31), height=60.97, radius=R, axis='z')
    s_ls_air_id = openmc.ZCylinder(x0=0, y0=0, r=r_calculated)

    target_region = -s_ls_air_od & +s_ls_air_id
    ...
    # Lazy Susan
    # New target cell replacing the original Air in Lazy Susan (kept naming convention to not have to modify most code)
    c_lazy_susan_air = openmc.Cell(fill=mats.np_aqueous, region=(-s_ls_air_od & +s_ls_air_id), name="Aqueous Np Target in Lazy Susan")
    ...
    c_G3 = openmc.Cell(fill=mats.water, region=(-s_G3),
                       name='Position G3')
    c_G4 = openmc.Cell(fill=make_element(704, s_G4, core_water_temperature, 'refl'),
                       region=(-s_G4), name='Position G4')
    c_G5 = openmc.Cell(fill=make_element(705, s_G5, core_water_temperature, 'refl'),
                       region=(-s_G5), name='Position G5')
    c_G6 = openmc.Cell(fill=make_element(706, s_G6, core_water_temperature, 'refl'),
                       region=(-s_G6), name='Position G6')
    c_G7 = openmc.Cell(fill=make_element(707, s_G7, core_water_temperature, 'refl'),
                       region=(-s_G7), name='Position G7')
    c_G8 = openmc.Cell(fill=make_element(708, s_G8, core_water_temperature, 'refl'),
                       region=(-s_G8), name='Position G8')
    c_G9 = openmc.Cell(fill=make_element(709, s_G9, core_water_temperature, 'refl'),
                       region=(-s_G9), name='Position G9')
    c_G10 = openmc.Cell(fill=make_element(710, s_G10, core_water_temperature, 'refl'),
                        region=(-s_G10), name='Position G10')
    c_G11 = openmc.Cell(fill=make_element(711, s_G11, core_water_temperature, 'refl'),
    ...
    c_G16 = openmc.Cell(fill=make_element(716, s_G16, core_water_temperature, 'refl'),
    ...
    c_G18 = openmc.Cell(fill=make_element(718, s_G18, core_water_temperature, 'refl'),
    ...
    np_tally.filters = [openmc.MaterialFilter(mats.np_aqueous)] 
