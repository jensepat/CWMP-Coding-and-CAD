...
openmc.config['chain_file'] = '/nfs/stak/users/bichselc/Paul TRIGA/slug_test/efficiency/chain_casl_pwr.xml' # This will have to be editted to your files for modifications
...
def ostr_model(n_particles, n_inactive, n_active, rod_heights, megawatt=False):
    ...
    def make_element(position, bounding_cyl, temp, element_type):
        ...
        if element_type == 'fuel':
            ...
            The target extends from the top of the bottom flute to the bottom of the top flute.
            It is 0.020" in thickness. There is assumed to be no radial gap between the components
            and the inner surface of the target.

            '''

            # Target Element Component Dimensions
            bottom_flute_height = 4.54
            target_height = 9.665 + 38.1 + 9.665
            void_gap_height = 1.27
            upper_flute_height = 2.54
            clad_height = target_height + void_gap_height

            # Build z-coordinates for bottom of cylinders for each component
            bottom_flute_z = z0
            target_z = bottom_flute_z + bottom_flute_height
            void_gap_z = target_z + target_height
            upper_flute_z = void_gap_z + void_gap_height
            ...
            s_bottom_flute = RCC(center_base=(x0, y0, bottom_flute_z), height=bottom_flute_height, radius=1.87325,
                                 axis='z',
                                 name=f"Bottom Flute of Reflector Element in Position {position}")
            s_target = RCC(center_base=(x0, y0, target_z), height=target_height, radius=1.82245, axis='z',
                             name=f"Target Meat in Position {position}")
            s_void_gap = RCC(center_base=(x0, y0, void_gap_z), height=void_gap_height, radius=1.82245, axis='z',
                             name=f"Expansion Gap of Reflector Element in Position {position}")
            s_upper_flute = RCC(center_base=(x0, y0, upper_flute_z), height=upper_flute_height, radius=1.87325, axis='z',
                                name=f"Top Flute of Reflector Element in Position {position}")
            s_cladding = RCC(center_base=(x0, y0, target_z), height=clad_height, radius=1.87325, axis='z',
                             name=f"Cladding of Reflector Element in Position {position}")

            # Cells for each component
            c_bottom_flute = openmc.Cell(fill=mats.flute_mix, region=(-s_bottom_flute),
                                         name=f"Bottom Flute of RE in Pos. {position}")
            c_target = openmc.Cell(fill=mats.target, region=(-s_target),
                                     name=f"Target Meat in Pos. {position}")
            c_void_gap = openmc.Cell(fill=mats.air, region=(-s_void_gap),
                                     name=f"Expansion Gap of RE in Pos. {position}")
            c_upper_flute = openmc.Cell(fill=mats.flute_mix, region=(-s_upper_flute),
                                        name=f"Top Flute of RE in Pos. {position}")
            c_cladding = openmc.Cell(fill=mats.steel,
                                     region=(-s_cladding & +s_target & +s_void_gap),
                                     name=f"Cladding of FE in Pos. {position}")

            # List of cells in fuel element
            target_cells = [c_bottom_flute,
                                  c_target,
                                  c_void_gap,
                                  c_upper_flute,
                                  c_cladding,
            ...
            # Make fuel element universe
            target_element = openmc.Universe(universe_id=position, name=f"Reflector Element in Position {position}",
                                           cells=target_cells)
    ...
    c_B3 = openmc.Cell(fill=make_element(203, s_B3, core_water_temperature, 'refl'),
    ...
    # E-Ring
    c_E1 = openmc.Cell(fill=make_element(501, s_E1, core_water_temperature, 'refl'),
    ...
    plot1 = openmc.Plot()
    plot1.filename = 'Center_XZ_Slice_Cells.png'
    plot1.width = (200, 150)
    plot1.basis = 'xz'
    ...
    mat_filter = openmc.MaterialFilter(mats.target)
    np_tally.filters = [mat_filter]
    np_tally.scores = ['absorption', '(n,gamma)']
...
def main():
    ...
    if args.run and not args.plot:
        print("Beginning OpenMC Run\n")
        openmc.run(threads=args.n_threads, cwd=args.directory)
        print("\n\n-- RUN COMPLETE --\n\n")

    if args.deplete:
        # Import materials here so we can access mats.target for the results extraction
        import OSTR_Materials as mats 
        ...
        print("\nBeginning OpenMC Depletion Calculation...")
        integrator.integrate()
        print("\n\n-- DEPLETION COMPLETE --\n\n")

        # --- EXTRACTING BATEMAN EQUATION RESULTS ---
        print("Extracting isotope inventories for the Np-237 -> Np-238 -> Pu-238 chain...\n")

        # Load the results from the HDF5 file generated by the integrator
        results = openmc.deplete.Results("depletion_results.h5")

        # Get the times and number of atoms for each isotope in the target material.
        # OpenMC automatically collapses the continuous-energy cross sections and 
        # fluxes to solve the Bateman matrix exponentially over the 30 days.
        times, np237_atoms = results.get_atoms(mats.target, 'Np237')
        times, np238_atoms = results.get_atoms(mats.target, 'Np238')
        times, pu238_atoms = results.get_atoms(mats.target, 'Pu238')

        # Calculate the requested values
        max_np238 = max(np238_atoms)
        max_pu238 = max(pu238_atoms)
        leftover_np237 = np237_atoms[-1] # Atoms at the final timestep

        print("================ 30-DAY BATEMAN RESULTS ================")
        print(f"Max Np-238 created:        {max_np238:.4e} atoms")
        print(f"Max Pu-238 created:        {max_pu238:.4e} atoms")
        print(f"Leftover Np-237:           {leftover_np237:.4e} atoms")
        print("========================================================")
